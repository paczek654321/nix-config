...
config = lib.mkIf config.my.hyprland.enable
{
	home-manager.users."${config.my.user.username}".wayland.windowManager.hyprland =
	{
		settings =
		{
			...
			workspace = concatMonitorsMap (w: m: "${workspaceIdx w m}, defaultName:${toString (w + 1)}, persistent:true, monitor:${builtins.elemAt monitors m}");
	...
	my.waybar.custom = lib.mkIf config.my.waybar.enable
	{
		"hyprland/workspaces" =
		{
			...
			}) workspaceCount) (builtins.genList (i: i) monitorCount))) // { default = ""; };
		};
	};
};
